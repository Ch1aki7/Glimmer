...
- 主线状态：进行中（条件分形生成 v2、旧场景兼容与固定视角视觉对照已完成；下一步是近远分层高度数据设计）
...
1. **已完成：固定视角视觉对照（2026-09-23）**。新增只读 RGBA8 帧缓冲导出与 Terrain Fixture 环境变量入口；同一 Alpine Seed、650 距离相机、光照和完整后处理下分别导出 v1/v2。v1 仍表现为中心丘体上均匀尖峰，v2 形成更连续、分层的定向山脊带；现有梯度核没有出现足以优先替换的方向伪影，因此保留并继续把预算用于地貌结构与侵蚀。水平默认尺寸从 256 扩为 1024 后，五个预设高度同步按四倍重标定，避免大视野中地形被压平；已序列化旧场景数值不变。
2. **已完成：分形地貌生成重写与数值验收（2026-09-23）**。v2 已建立低频区域、定向山带、条件山脊/地质/谷地和按像素足迹过滤的细节；旧 YAML 固定回退 v1，预设保留版本。真实 GPU 验证覆盖有限值、范围、派生权重、重复生成哈希和 Shader 编译。
3. **当前实施项：近远分层高度数据设计**。解决大范围单张 HeightMap 的近景欠采样，并给出显存、生成、水文模拟成本边界。
4. 在目标显卡上验证大视距颜色/阴影/水面提交量与 GPU 帧时，按证据调整默认质量和 LOD；补齐连续几何过渡或可接受的视觉证据。
5. 完整构建、回归、真实 GPU 运行和三份文档同步后，完成 P16 总体验收。
