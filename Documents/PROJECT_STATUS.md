...
- 当前默认验证配置：`Debug | x64`
- 当前主线：P9 方向光阴影与 CSM
- 主线状态：进行中（1～4 级 CSM 基础链路已落地，下一阶段为级联过渡、剔除与调试可视化）
...
- Directional Light Inspector 现提供 Cast Shadows、512～4096 Resolution、1～4 Cascade Count、Shadow Distance、Bias 与 Split Lambda；设置进入 Scene YAML，级联深度纹理、FBO、矩阵和 Split 保持纯运行时；
- 验证：VS2026 生成新版编辑器目标，独立回归目标构建成功；55 项断言及最终汇总全部 PASS，其中级联数与 Split Lambda 完成场景往返；Intel Iris Xe/OpenGL 4.6 下 `ShadowDepth`、PBRModel、Terrain 与三个 Terrain Compute Shader 均编译成功，PBR Lab 渲染 6/6 项且无跳过；
- 尚未完成：级联边界混合、Shadow Frustum 剔除、级联调试可视化、Alpha Mask 投影契约、Shadow Instancing 和独显实机画质调参，因此 P9 继续保持进行中。
...
- CSM 已有 Practical Split 和 Texel Snap，但级联边界仍为硬切换，且没有 Shadow Frustum 剔除、级联调试视图和 Alpha Mask 投影；模型 Shadow Pass 也尚未复用 Instancing，均属于 P9 后续阶段；
