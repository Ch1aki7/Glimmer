...
PBRModel 与 Terrain 共用四组 Light VP、Cascade Split/Blend Width、Camera View、Bias 和 `3×3 PCF` 接收契约，并按片元视空间深度选择级联。Split 过渡区同时采样相邻级联并以 `smoothstep` 混合，区间外只采样当前级联。模型材质占用 0～3 后将级联图绑定到 slot 4～7；Terrain 已占用 0～15，级联图绑定到 slot 16～19。Shadow Framebuffer、深度纹理、Light VP、Split 与 Blend Width 不序列化，DirectionalLight 只持久化重建所需设置。Shadow Pass 仍逐实体提交，但每级只绘制与其 Clip Frustum 保守相交的子网格/地形；尚未实现 Alpha Mask 投影或 Shadow Instancing。
...
近期架构演进顺序以 `Documents/PROJECT_STATUS.md` 为唯一来源。3D Instancing、MaterialInstance 缓存、Transparent RenderQueue、AlphaMode、PBR Normal/AO/Emissive 通道、无窗口回归入口、Terrain 生命周期/Inspector 事务、山脉生成/派生图/有限次 Authoring Erosion、TerrainMaterial 四层 Triplanar PBR，以及 1～4 级方向光 CSM 基础链路已经落地；当前主线继续处理级联过渡、剔除与调试能力。Metallic/Roughness Texture/ORM、Runtime Erosion、IBL、Chunk/LOD 和环境模拟目前均是未实现或未完整实现的后续能力，不应从本文件推断为已经落地。Vulkan 后端继续保持接口预埋状态，不阻塞当前 OpenGL 主线。
