...
面板第一次绘制时才解析 `assets/` 的绝对路径，构造 `EditorLayer` 时不会提前遍历磁盘。左边是递归目录树，右边可以在紧凑列表和缩略图网格之间连续缩放；中间的 4 像素分隔条可以拖动，树和文件区域各自滚动。

```text
Content Browser
  assets 目录树     |     当前目录文件列表 / 网格
  单击切换目录      |     单击选择 / 双击打开 / 拖放
                    |                    [缩放滑块]
             可拖动分隔线
```

目录树只展开用户点开的节点，叶子目录不画多余箭头。回退按钮到达 `assets/` 后停止，面板内部的导航路径不会越过项目资产根。文件区域每帧直接遍历当前目录，没有维护另一份文件缓存，所以在外部新增文件后通常能马上看到；显示时按文件夹优先、名称不区分大小写排列。

贴近右下边框的紧凑滑块控制条目缩放。外层右栏不滚动，只有文件区域持有滚动条；文件区域的下边缘直接取滑块顶部，因此不会再出现第二条外层滚动条或两者之间的大段空白。在文件区域悬停时也可以按住 `Ctrl` 滚动鼠标滚轮。滑块位于最小端时，每个条目变为与文件区域同宽的窄行，从上到下排列；离开最小端后恢复方形网格，并在 `72～160` 像素之间连续改变方格和图标尺寸。方格上方由图标占据主要空间，文件名固定在底部一行；过长名称根据实际像素宽度显示省略号，悬停条目可查看完整名称。

文件夹以及 Shader、Model、Image、Scene、Material、TerrainMaterial、Skybox 和普通文件使用不同颜色与内部标记。图标不是预缩放的字符或位图，而是由 ImGui DrawList 的矩形、折角、线条和圆形实时绘制，因此紧凑行与大方格共用同一套清晰图形，也不需要增加图片资源及其资产注册项。缩放值只属于本次面板运行时状态，不进入 Scene 或 Asset Registry。
...
这个面板仍是直接文件系统视图，没有搜索、过滤、重命名和删除工作流。当前排序只提供文件夹优先和不区分大小写的名称顺序。目录树为了判断叶子节点会额外扫描子目录，右侧也会逐帧遍历并排序当前目录；目前资产规模不大，做法简单够用，但不适合直接扩展到很大的内容库。
